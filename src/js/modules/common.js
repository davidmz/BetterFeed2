...
module.watch(".timeline-post, .single-post", (node) => {
  ...
  if (
    isArabic(postBody.textContent) ||
    (firstComment && isArabic(firstComment.textContent))
  ) {
...
module.watch(".user-card .display-name", (node) => {
  closestParent(node, ".user-card").dataset.userName = node
    .getAttribute("href")
    .substr(1);
});
