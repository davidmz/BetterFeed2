...
export default class {
  ...
  unlike(commId) {
    ...
    return this.req(`${apiRoot}/unlike?comm_id=${encodeURIComponent(commId)}`, {
      method: "POST",
    });
  }

  async allLikes(commIds) {
    const likes = await this.req(`${apiRoot}/all-likes`, {
      method: "POST",
      body: JSON.stringify(commIds),
    });
  ...
  async allPostsLikes(postIds) {
    const likes = await this.req(`${apiRoot}/all-posts-likes`, {
      method: "POST",
